...
- [core] the browser-only `EnvVariablesServer` stub now returns `file:///.theia` from `getConfigDirUri()`, instead of an empty string that resolved to the OPFS root. Browser-only application state moves from the OPFS root to `/.theia`, and the UTF-8 encoding override in `CommonFrontendContribution` is scoped to `/.theia` rather than the whole file tree. Existing deployments are not migrated automatically; see `doc/Migration.md` [#14776](https://github.com/eclipse-theia/theia/pull/14776)
